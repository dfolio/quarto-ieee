...
\
Ipsum habitant inceptos volutpat cras nostra, proin commodo eget varius. Volutpat arcu praesent, nisi vulputate convallis inceptos consequat. 
Aenean hendrerit sociis congue nulla ultricies, at tempor sapien nullam ullamcorper. 
...
\
It is recommended to avoid using the commonly used single markdown table known as a 'pipe table'. 
In fact, Pandoc markdown uses the {{< latex >}} `longtable` package, which does not support the two-column mode[^issue-1023], which is required for most `IEEEtran` journals.
...
[^issue-1023]: <https://github.com/jgm/pandoc/issues/1023>
...
The figures is cross-referenced as @tbl-panel for details, especially @tbl-second.
