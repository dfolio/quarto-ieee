...
author:
  - name: David Folio
    ...
    attributes:
      corresponding: true
      note: "Template created June 23, 2023; revised October 11, 2023."
  - name: John Doe
abstract: |
...
It is assumed that the reader has a basic working knowledge of {{< latex >}} and of (Pandoc/Quarto)-markdown [@mittelbach2023latex; @MacFarlane_Pandoc; @Allaire_Quarto_2022].


# The Design, Intent, and Limitations of the Templates

The templates are intended to **approximate the final look and page length of the articles/papers**. 
**They are NOT intended to be the final produced work that is displayed in print or on IEEEXplore^®^**.
They will help to give the authors an approximation of the number of pages that will be in the final version. 

Although most of the {{< latex >}} and `IEEEtran.cls` commands and environment are supported, 
there are some limitations when trying to export to a format other than PDF.
...
:::{.IEEEbiography options="\includegraphics{david-folio}" arguments="David Folio"}
Use `IEEEbiography` div 
with figure as  option and
the author name as the argument followed by the biography text.
:::

:::{.IEEEbiographynophoto arguments="John Doe"}
Use `IEEEbiographynophoto` div and the author name as the argument followed by the biography text.
:::
